module.exports = {
    ...
    insertBlog: 'Insert into blog (heading, content, image, view_option, user_id) values (?,?,?,?,?)',
    //select statements
    ...
    selectDataFromID: 'SELECT firstname, lastname FROM  users INNER JOIN blog ON users.user_id=blog.user_id WHERE  users.user_id=?',
    // selectDataFromID: 'SELECT firstname FROM  users INNER JOIN blog ON users.user_id=blog.user_id WHERE  users.user_id=blog.user_id',
